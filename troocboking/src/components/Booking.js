...
export default function Booking({ id }) {
    ...
    if (!movieDetails) {
        return <div className='w-full h-[462px] flex justify-center bg-[#1C1B21] rounded-[20px] p-[20px] text-white'>Choose Film to check the seats</div>;
    }
        ...
        <div className='w-full'>
            <div className="w-full h-[462px] bg-[#1C1B21] rounded-[20px] p-[20px]">
                <div className="w-full flex justify-between">
                    <div>
                        <p className="font-bold font-mono text-[16px] text-white">{movieDetails.title}</p>
                    </div>
                    <div className='flex gap-[5px]'>
                        <div>
                            <select
                                ...
                                onChange={(e) => handleSlotChange(movieDetails.slot.find(slot => slot.time === e.target.value))}
                            >
                ...
                <div className='w-full flex justify-center items-center mt-[20px]'>
                    <div className='grid grid-cols-7 gap-2'>
                        {seats.map((seat) => (
                            <div key={seat.seat} className="flex justify-center items-center w-8 h-8 rounded-[5px]" onClick={() => handleSeatClick(seat.seat)}>
